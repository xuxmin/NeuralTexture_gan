...
cfg = "experiments/newganpipeline_batch1_Adam_lr1e-3_tex256_f16_alldata_loss-l1_augment_debug.yaml"
...
def debug_model(model, eggDataset, epoch, nth, checkpoint_dir, logger):
    ...
    with torch.no_grad():
        ...
        light_dir = np.asarray([0.0, 0.0, 0.6])

        for i in range(360):
            theta = math.radians(i)
            light_dir[0] = math.sin(theta) * 0.8
            light_dir[1] = math.cos(theta) * 0.8
            ...
            cv2.imwrite("output/temp.png", pad_img)
            temp = cv2.imread("output/temp.png")
    ...
    # logger.info('debug model finished!')
...
def main():    
    ...
    # optimizerD = torch.optim.SGD(netD.parameters(), lr=D_LR, momentum=0.8)
    ...
    for epoch in range(configs.TRAIN.BEGIN_EPOCH, configs.TRAIN.END_EPOCH):
        ...
        for i, (uv_map, gt, masks, normal, light_pos, view_dir) in enumerate(train_loader):
            ...
            if i % 20 == 0:
                logger.info('[%d][%d/%d] Loss_L1: %.4f(%.4f) Loss_VGG: %.4f(%.4f)'
                            % (epoch, i, len(train_loader), errL1.item(), loss_errL1.avg, errVGG.item(), loss_errVGG.avg))
